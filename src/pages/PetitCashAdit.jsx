  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--color-surface)', borderRadius: 8, boxShadow: 'var(--shadow)', overflow: 'auto', maxHeight: '70vh' }}>
        <table style={{ whiteSpace: 'nowrap', fontSize: 12.5 }}>
          <thead>
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id}>
                  <td><button onClick={() => startEdit(r)} style={btnGhost}>Bearb.</button></td>
                  <td>{formatDatum(r.datum)}</td>
                  <td style={{ overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 240 }}>{r.beschreibung}</td>
                  <td>{r.konto}</td>
                  ...
                  <td>{r.status}</td>
                  <td style={{ fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 160 }}>{r.notiz || '—'}</td>
                  <td>
                    {urls.length > 0
